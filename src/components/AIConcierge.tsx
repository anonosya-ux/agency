  ...
  return (
    <>
      <motion.button 
        ...
        className="fixed bottom-6 right-6 w-14 h-14 bg-dark rounded-full text-accent flex items-center justify-center shadow-2xl z-50 border border-text/10 hidden md:flex"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="bg-dark p-4 flex justify-between items-center text-text">
            ...
            <div className="flex-1 p-4 overflow-y-auto bg-gray-50 flex flex-col gap-3 text-sm">
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] p-3 ${m.role === 'user' ? 'bg-dark text-text' : 'bg-white border border-[#EAEAEA] text-dark'}`}>
                    {m.text}
                  </div>
                </div>
              ))}
            ...
            <div className="p-3 bg-white border-t border-[#EAEAEA] flex items-center gap-2">
              ...
              />
              <button onClick={handleSend} className="text-dark hover:text-accent transition-colors">
