...
export function ElevatorScroll() {
  ...
  return (
    <div ref={containerRef} className="relative h-[400vh] bg-white">
      <div className="sticky top-0 h-screen w-full overflow-hidden text-dark flex items-center justify-center">
        ...
        >
          <div className="overflow-hidden">
            <motion.p initial={{y: "100%"}} animate={{y: 0}} transition={{delay: 1, duration: 1}} className="text-xs uppercase tracking-[0.3em] mb-6 text-accent font-medium">Фатюхин и Ко • Агентство Недвижимости</motion.p>
          </div>
          ...
          <div className="overflow-hidden mt-8">
            <motion.p initial={{y: "100%"}} animate={{y: 0}} transition={{delay: 1.5, duration: 1}} className="max-w-2xl text-lg font-light text-dark/80 mix-blend-difference">
        ...
        >
          <motion.div className="w-full md:w-1/2 md:pl-24" style={{ y: s2Y }}>
            <span className="text-xs uppercase tracking-[0.2em] block mb-4 text-accent flex items-center gap-4">
              <span className="w-12 h-[1px] bg-accent" /> Топ Локации
            </span>
            <h2 className="font-serif text-[8vw] md:text-7xl leading-[0.9] tracking-tighter mb-8">Новостройки<br/>Бизнес-класса</h2>
            <p className="text-dark/70 font-light text-xl mb-10 max-w-md">Современные жилые комплексы в развитых районах. Удобные планировки, паркинги и инфраструктура для комфортной жизни.</p>
            <a href="#catalog" data-magnetic="true" className="group relative overflow-hidden border border-dark text-dark px-10 py-5 uppercase tracking-widest text-xs font-medium transition-colors inline-block">
              <span className="relative z-10 group-hover:text-text transition-colors duration-500">Смотреть лоты</span>
              <div className="absolute inset-0 bg-dark transform scale-x-0 origin-left group-hover:scale-x-100 transition-transform duration-500 ease-[0.76,0,0.24,1]" />
            </a>
          </motion.div>
        ...
        <motion.div 
          ...
          className="absolute inset-0 bg-light-gray flex flex-col items-center justify-center md:flex-row-reverse gap-12 px-8 max-w-[100vw] mx-auto z-30"
        >
          <motion.div className="w-full md:w-1/2 md:pr-24" style={{ y: s3Y }}>
            <span className="text-xs uppercase tracking-[0.2em] block mb-4 text-accent flex items-center md:justify-end gap-4">
              Загородная <span className="w-12 h-[1px] bg-accent" />
            </span>
            <h2 className="font-serif text-[8vw] md:text-7xl leading-[0.9] tracking-tighter mb-8 md:text-right">Загородные<br/>Дома</h2>
            <p className="text-dark/70 font-light text-xl mb-10 max-w-md md:ml-auto md:text-right">Коттеджи и таунхаусы в экологически чистых направлениях Подмосковья для всей семьи.</p>
            <div className="flex md:justify-end">
              <a href="#catalog" data-magnetic="true" className="group relative overflow-hidden border border-dark text-dark px-10 py-5 uppercase tracking-widest text-xs font-medium transition-colors inline-block">
                <span className="relative z-10 group-hover:text-text transition-colors duration-500">Изучить проекты</span>
                <div className="absolute inset-0 bg-dark transform scale-x-0 origin-right group-hover:scale-x-100 transition-transform duration-500 ease-[0.76,0,0.24,1]" />
              </a>
            </div>
          </motion.div>
          <div className="w-full md:w-1/2 aspect-[3/4] md:aspect-[4/5] relative overflow-hidden">
        ...
        <motion.div 
          ...
          className="absolute inset-0 bg-accent flex flex-col items-center justify-center text-center px-4 z-40 text-text"
        >
          ...
          <a href="#contacts" data-magnetic="true" className="group relative overflow-hidden bg-white text-dark px-12 py-6 uppercase tracking-[0.3em] text-xs font-bold shadow-2xl transition-transform hover:scale-105 duration-500 inline-block">
